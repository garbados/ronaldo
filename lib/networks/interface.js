...
/**
The network's cost function for an individual neuron,
given the desired output and that neuron's actual output
for the same input.

Defaults to the method used in harthur/brain:
https://github.com/harthur/brain/blob/master/lib/neuralnetwork.js#L155

@function
@param {number} target - desired output bit
@param {number} output - actual output bit
@returns {number} the "cost" of the difference between the target and the output
*/
Network.prototype._cost_function = function (target, output) {
  ...
  this.layers().slice(1).reverse().forEach(function (layer, l) {
    ...
    layer.neurons().forEach(function (neuron, j) {
      if (i === outputs.length-1) {
        // output layer
        errors[0][j] = self._cost_function(outputs[i][j], target[j]);
      } else {
