...
Network.prototype._calculate_deltas = function (outputs, target) {
  ...
  var errors = [];
  this.layers().reverse().slice(0, -1).forEach(function (layer, l) {
    ...
    layer.neurons().forEach(function (neuron, j) {
      if (i === outputs.length-1) {
        ...
        // hidden layers
        // n.b. input layers do not learn, they only pass what's given
        ...
        errors[0][j] = deltas.map(function (delta, k) {
          ...
          return self.layer(i-1).neurons().map(function (neuron, m) {
            return neuron.weight(k);
          }).reduce(function (a, b) {
            return a + b;
          }, delta);
        }).reduce(function (a, b) {
